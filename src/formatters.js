	...
	for (const [key, value] of Object.entries(changes)) {
		...
		if (
			...
			if (value.added?.length) {
				messages.push(
					`➕ **${value.added.length}** new item(s) added to **${fieldName}**`,
				);
			...
			if (value.removed?.length) {
				messages.push(
					`➖ **${value.removed.length}** item(s) removed from **${fieldName}**`,
				);
			...
			if (value.modified?.length) {
				messages.push(
					`✏️ **${value.modified.length}** item(s) modified in **${fieldName}**`,
				);
				value.modified.forEach((item) => {
					...
					const subMessages = formatDiffMessage(
						item.diff,
						currentPath,
						gameName,
						gameConfig,
